...
export default function BoardPage() {
  ...
  async function handleAddTask(columnId: string, title: string, priority: TaskPriority) {
    const res = await taskApi.create(slug, projectId, columnId, { title, priority });
  ...
  return (
    <>
      <PageHeader
        title={project.name}
        routerLink={`/workspace/${slug}/projects/${projectId}`}
        slug={slug}
        ...
        <div className="grid gap-28 overflow-x-auto pb-4 md:grid-cols-4">
          {project.board.columns.map((col) => (
            <BoardColumn
              ...
              onAddTask={(title, priority) => handleAddTask(col.id, title, priority)}
            />
          ))}
        </div>
